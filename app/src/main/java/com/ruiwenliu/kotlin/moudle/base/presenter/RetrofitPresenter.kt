...
import com.ruiwenliu.kotlin.moudle.base.http.HttpApi
import com.ruiwenliu.kotlin.moudle.base.http.OkHttpCreate
...
abstract class RetrofitPresenter<V : BaseView> : WrapperPresenter<V>() {
     var RETRY_COUNT:Int = 0//请求失败重连次数
  ...
  private  var disMap :MutableMap<Any, DisposableObserver<*>>? = null
    ...
    fun<T> toSubscribe(
        ...
        observable.subscribeOn(Schedulers.io())
            ...
            .subscribe(getDisMap()?.get(position) as DisposableObserver<T>)
    }

    fun getDisMap(): MutableMap<Any, DisposableObserver<*>> {
        if (disMap == null) {
